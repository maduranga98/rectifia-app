...
// v1's only source for a company's headcount used by this UI: a number the
// Company Admin types in themselves, not derived from the
// companies/{companyId}/employees roster (that count backs the separate,
// pre-existing Stripe-metered subscription - see BillingQuote.jsx). Shown
// inline so declaring/editing it never leaves this card.
...
// component.
//
// This only ever writes companies/{companyId}.subscriptionTier /
// .subscriptionUpdatedAt via upgradeSubscription.js - it does not implement
// payment processing, and it does not touch the pre-existing Stripe-driven
// subscription BillingQuote.jsx/BillingPage.jsx show above this card (see
// upgradeSubscription.js's file comment for why these are deliberately
// separate, parallel tracks).
//
// TODO(proration): band changes here apply immediately with no prorated
// credit or charge - see upgradeSubscription.js's matching TODO. The
// immediate-effect note below must stay visible until that lands, so an
// admin switching bands mid-cycle isn't misled into expecting a fair
...
function PackageSelector({ companyId, company, onChanged }) {
  ...
  async function handleSelect(tier) {
    ...
    setError(null)
    try {
      await upgradeSubscription(companyId, { target: 'core', tier })
      await onChanged()
    } catch (err) {
      setError(err.message)
    } finally {
